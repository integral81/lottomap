...
try:
    ...
    new_lines = []
    for line in lines:
        if f'name: "{target_name}"' in line and (target_addr_part in line or "대야1로" in line):
            new_line = f'            {{ name: "{target_name}", addr: "경기 군포시 대야1로", panoId: {panoid}, pov: {{ pan: {pan}, tilt: {tilt}, zoom: {zoom} }} }},\n'
