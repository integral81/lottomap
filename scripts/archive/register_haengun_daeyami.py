...
try:
    ...
    new_lines = []
    for line in lines:
        if f'name: "{target_name}"' in line and (target_addr_part in line or "대야미동" in line):
            new_line = f'            {{ name: "{target_name}", addr: "경기 군포시 대야미동", panoId: {panoid}, pov: {{ pan: {pan}, tilt: {tilt}, zoom: {zoom} }} }},\n'
