...
# Batch 16 Data
batch16 = [
    { "name": "베스트원", "addr": "전남 영광군 영광읍 남천리 315 CU편의점", "panoid": 1192687174, "pov": { "pan": 275.17, "tilt": -4.79, "zoom": 1 } },
...
for s in data:
    for b in batch16:
        # Check name and partial address match to avoid false positives
        if b['name'] in s.get('n', ''):
            # Special case for "복권판매소(서문큰장네거리)" - name in DB might be just "복권판매소"
            # So we check if address part matches too if possible, or just proceed if name is unique enough?
            # Actually, "복권판매소" is very generic. Let's check address for that one.
