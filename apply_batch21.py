...
# Batch 20 Data (13 items) - Re-adding since user cancelled previous step
batch20 = [
...
combined_batch = batch20 + batch21
...
def main():
    ...
    for b in combined_batch:
        ...
        # Address keyword for better matching (city/gu)
        # e.g. "경기 수원시 권선구..." -> "권선구"
        ...
        for s in data:
            ...
            # Name match + Optional Address hint
            if (n1 in n2 or n2 in n1):
                 # Confirmatory check if generic name
                 if "복권" in n1 and len(n1) < 5 and addr_keyword and addr_keyword not in s.get('a', ''):
                     continue # Skip mismatched generic names
    ...
    save_data(data)
    print(f"Combined Batch (20+21): Updated {updated_count} / {len(combined_batch)} shops.")
    if not_found:
